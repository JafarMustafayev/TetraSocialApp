...
public class RegisterRequestValidator : AbstractValidator<RegisterRequestDto>
{
        ...
        IAppConfig appConfig)
    {
        ...
        RuleFor(x => x.Email)
            ...
            .WithMessage(
                localizer.Get("Validation.Common.MaxLength", "Email", new Dictionary<string, object>
                {
        ...
        RuleFor(x => x.Username)
            ...
            .WithMessage(localizer.Get("Validation.Common.Required", "Username"))
            .MinimumLength(rules.UserName.MinLength ?? 0).When(_ => rules.UserName.Required)
            .WithMessage(
                localizer.Get("Validation.Common.MinLength", "Username", new Dictionary<string, object>
                {
                    ["MinLength"] = rules.UserName.MinLength ?? 0
                }))
            .MaximumLength(rules.UserName.MaxLength ?? int.MaxValue).When(_ => rules.UserName.Required)
            .WithMessage(
                localizer.Get("Validation.Common.MaxLength", "Username", new Dictionary<string, object>
                {
        ...
        RuleFor(x => x.FirstName)
            ...
            .WithMessage(localizer.Get("Validation.Common.Required", "FirstName"))
            .MinimumLength(rules.FirstName.MinLength ?? 0).When(_ => rules.FirstName.Required)
            .WithMessage(
                localizer.Get("Validation.Common.MinLength", "FirstName", new Dictionary<string, object>
                {
            ...
            .WithMessage(
                localizer.Get("Validation.Common.MaxLength", "FirstName", new Dictionary<string, object>
                {
        ...
        RuleFor(x => x.LastName)
            ...
            .WithMessage(localizer.Get("Validation.Common.Required", "LastName"))
            .MinimumLength(rules.LastName.MinLength ?? 0).When(_ => rules.LastName.Required)
            .WithMessage(
                localizer.Get("Validation.Common.MinLength", "LastName", new Dictionary<string, object>
                {
                    ["MinLength"] = rules.LastName.MinLength ?? 0
                }))
            .MaximumLength(rules.LastName.MaxLength ?? int.MaxValue).When(_ => rules.LastName.Required)
            .WithMessage(
                localizer.Get("Validation.Common.MaxLength", "LastName", new Dictionary<string, object>
                {
